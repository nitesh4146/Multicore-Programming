...
Window* find(atomic<Node> *head, int key){
	atomic<Node>* pred;	Node* curr = NULL;	Node* succ = NULL;
	bool marked[] = {false};
	bool snip;
	retry: while(true){
		pred = head; //head atomic
		//curr = pred->next; //pred.next.getReference()
		while(true){
			succ = curr->next; //curr.next.get(marked);
			while(marked[0]){
				snip = atomic_compare_exchange_weak(pred->next, curr, succ); //pred.next.compareAndSet(curr, succ, false, false)
				if(!snip) goto retry;
				curr = succ;
				succ = curr->next; //curr.next.get(marked)
			}
