...
void can_I_eat(phil p,int c1, int c2){
		if(queue_empty()){
			if(sticks_avilable(c1,c2)){
				grab_chopstick(c1,c2);
			}else{
				enqueue(p);
			}
		}
...
sticks_available(int c1, int c2){
	if(pthread_mutex_trylock(&chopstick[c1]);
}
